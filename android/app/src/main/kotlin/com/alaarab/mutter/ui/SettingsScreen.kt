...
@Composable
fun SettingsScreen(app: MutterApplication, settings: Settings) {
    ...
    if (backOwner != null)
        CompositionLocalProvider(LocalOnBackPressedDispatcherOwner provides backOwner) {
            BackHandler(page != "main") { page = "main" }
        }
    AnimatedContent(
        page,
        transitionSpec = {
            ...
            Column(Modifier.fillMaxWidth()) {
                if (current != "main")
                    Row(
                        ...
                        ActionIcon(Icons.AutoMirrored.Rounded.ArrowBack, "Back to settings") {
                            page = "main"
                        }
                        TextButton({ page = "main" }) { Text("Settings") }
                    }
                if (current == "identities") IdentitiesScreen(app)
                    ...
                    LazyColumn(
                        ...
                        when (current) {
                            "main" -> {
                                item { Heading("Make it yours", "A space that feels like you.") }
                                item {
                                    AppCard(Modifier.fillMaxWidth()) {
                                        SettingsLink(
                                            Icons.Rounded.GraphicEq,
                                            "Voice & audio",
                                            "${voiceModeLabel(settings.voiceMode)} · ${settings.bitrate / 1000}\u00a0kbps",
                                        ) {
                                            page = "voice"
                                        }
                                        SectionDivider()
                                ...
                                item { ThemePicker(settings) { save(settings.copy(theme = it)) } }
                                item { SectionLabel("Comfort") }
                                item {
                                    AppCard(Modifier.fillMaxWidth()) {
                                        ToggleRow(
                                            "Hide empty channels",
                            ...
                            "voice" -> {
                                item { Heading("Voice & audio", "Sound clear. Feel close.") }
                                item { SectionLabel("Microphone") }
                                item {
                                    AppCard(Modifier.fillMaxWidth()) {
                                ...
                                item { SectionLabel("Audio quality") }
                                item {
                                    ChoiceRow(
                                ...
                                item { SectionLabel("Playback & processing") }
                                item {
                                    AppCard(Modifier.fillMaxWidth()) {
                                        ToggleRow(
                                            "Speakerphone",
                            ...
                            "viewer" -> {
                                item {
                                    Heading(
                                        "Screen viewer",
                                        "Stay in the conversation while you watch.",
                                    )
                                }
                                item {
                                    AppCard(Modifier.fillMaxWidth()) {
...
private fun VoiceModeRow(mode: String, selected: Boolean, choose: () -> Unit) {
    Row(
        ...
        Column(Modifier.weight(1f)) {
            Text(voiceModeLabel(mode), style = MaterialTheme.typography.titleSmall)
            Hint(
                when (mode) {
