...
try {
  ...
  await step('Alpha starts sharing a 640×360 canvas stream with an audio track', async () => {
    ...
    await a.waitFor(`document.getElementById('shareBtn').classList.contains('active') && !document.getElementById('screenBtn').hidden && document.body.dataset.tab === 'screen'`);
  });
  ...
  await step('Bravo is offered the share (announce arrived, card and badge shown)', async () => {
    ...
    await b.waitFor(`!!document.querySelector('.offer .watch') && !!document.querySelector('.user .live-badge') && !document.getElementById('screenBtn').hidden`);
  ...
  await step('Stop sharing clears the offer everywhere', async () => {
    ...
    await b.waitFor('mutter.share.available.size === 0 && document.getElementById("screenBtn").hidden && document.body.dataset.tab !== "screen"', { timeout: 4000 });
  });
